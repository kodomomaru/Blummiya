...
import { Share2, Sparkles, Flame, User, ArrowRight, Heart } from 'lucide-react';
...
export function TrailsExplorer({ onSelectTrailSkill }: TrailsExplorerProps) {
  ...
  return (
    <div className="max-w-5xl mx-auto py-6 px-4 space-y-6">
      {/* Header Banner */}
      <div className="p-6 rounded-2xl glass-panel border border-white/10 text-left">
        <div className="flex items-center space-x-2 text-amber-400 text-xs font-semibold uppercase tracking-wider mb-1">
          <Share2 className="w-4 h-4" />
          <span>Shared Knowledge Trails</span>
        </div>
        <h2 className="text-2xl font-bold text-white tracking-tight">Leaving Trails to Light the Way</h2>
      ...
      {/* Trails Grid */}
      {loading ? (
        <div className="py-16 text-center text-sm text-slate-500">Discovering shared trails...</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {trails.map((trail) => {
            ...
            return (
              <div
                key={trail.id}
                className="flex flex-col justify-between p-6 rounded-2xl bg-[#0d131f]/80 border border-white/10 hover:border-amber-500/30 transition-all glass-panel-hover"
              >
                <div>
                  ...
                  <h3 className="text-lg font-bold text-white tracking-tight">{trail.title}</h3>
                  <p className="text-xs sm:text-sm text-slate-300 mt-2 leading-relaxed">{trail.description}</p>
                        ...
                        return (
                          <React.Fragment key={skill.id}>
                            <button
                              ...
                              className={`text-xs px-2.5 py-1 rounded-lg border font-medium cursor-pointer transition-all hover:scale-105 ${cat.bgClass} ${cat.borderClass}`}
                ...
                <div className="mt-6 pt-4 border-t border-white/5 flex items-center justify-between text-xs text-slate-400">
                  <span className="italic text-[11px]">"Grow forward. Light the way."</span>
                  <button
                    onClick={() => handleLightTrail(trail.id)}
                    className="text-amber-400 hover:text-amber-300 font-medium flex items-center space-x-1 transition-colors"
                  >
