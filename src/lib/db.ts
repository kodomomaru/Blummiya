...
function initDatabase(db: DatabaseSync) {
  ...
  // Seed default core constellation if table is empty
  const countStmt = db.prepare('SELECT COUNT(*) as count FROM skills');
  const result = countStmt.get() as { count: number };
  if (result.count === 0) {
    seedInitialPathway(db);
  }
}

function seedInitialPathway(db: DatabaseSync) {
  const now = new Date().toISOString();

  // Baseline skills across 4 organic domains
  const seedSkills = [
    {
  ...
  const insertMomentSkill = db.prepare(`
    INSERT INTO moment_skills (moment_id, skill_id, contribution)
    VALUES (?, ?, ?)
  `);
  ...
  // Seed inspirational shared trail
  const insertTrail = db.prepare(`
    INSERT INTO trails (id, title, author_name, description, skill_ids, is_shared, sparks_count, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `);
